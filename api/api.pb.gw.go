...
func RegisterOpenStorageCloudDriveSetHandlerClient(ctx context.Context, mux *runtime.ServeMux, client OpenStorageCloudDriveSetClient) error {
	...
	mux.Handle("PUT", pattern_OpenStorageCloudDriveSet_Detach_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(req.Context())
		defer cancel()
		if cn, ok := w.(http.CloseNotifier); ok {
			go func(done <-chan struct{}, closed <-chan bool) {
...
var (
	pattern_OpenStorageCloudDriveSet_Detach_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2}, []string{"v1", "clouddrives", "id"}, ""))
)
